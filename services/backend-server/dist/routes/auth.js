...
const BUCKET_SECRET_ACCESS_KEY = process.env.BUCKET_SECRET_ACCESS_KEY || "default-secret-access-key";
...
router.post("/profile/avatar", upload.single("avatar"), async (req, res) => {
    try {
        ...
        const fullSizePresignedUrl = await getSignedUrl(s3, new GetObjectCommand({
            Bucket: BUCKET_NAME,
            Key: fullSizeKey,
        }), { expiresIn: 86400 });
        const smallSizePresignedUrl = await getSignedUrl(s3, new GetObjectCommand({
            Bucket: BUCKET_NAME,
            Key: smallSizeKey,
        }), { expiresIn: 86400 });
