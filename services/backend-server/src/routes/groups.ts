...
import { z } from "zod";
import Group, { type GroupDoc } from "../models/Groups.js";
...
router.get("/:groupId", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const userRole = await getUserRoleInGroup(groupId as string, userId);
    if (!userRole) {
      return res
        .status(403)
...
router.post("/:groupId/members", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingMember = group.members.find(
      (m: any) => m.id.toString() === newMemberId
    );
    if (existingMember) {
      return res
        .status(400)
...
router.put(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const memberIndex = (group.members as any).findIndex(
      ...
      const currentMemberRole = (group.members as any)[memberIndex].role;
      ...
      if (!hasPermission(userRole, "update_member_role", currentMemberRole)) {
        return res.status(403).json({
          message:
            "Access denied: Insufficient permissions to update this member's role",
        });
      }

      // Update member role
      (group.members as any)[memberIndex].role = newRole;
      await group.save();

      return res.status(200).json({
        message: "Member role updated successfully",
        member: (group.members as any)[memberIndex],
      });
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const targetMemberRole = (group.members as any)[memberIndex].role;
      ...
      if (!hasPermission(userRole, "remove_member", targetMemberRole)) {
        return res.status(403).json({
          message:
            "Access denied: Insufficient permissions to remove this member",
        });
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      (group.pendingInvitations as any).push(invitation._id);
      await group.save();
      ...
      if (inviteeUser) {
        ...
        (inviteeUser.pendingInvitations as any).push(invitation._id);
 ...
 */
router.get("/:groupId/invitations", async (req: AuthRequest, res: Response) => {
  try {
    const { groupId } = req.params;
    const userId = req.user?.id;
    ...
    try {
      ...
      if (inviteeUser) {
        inviteeUser.pendingInvitations =
          (inviteeUser.pendingInvitations as any)?.filter(
            (id: any) => id.toString() !== invitationId
          ) || [];
        await inviteeUser.save();
      }
